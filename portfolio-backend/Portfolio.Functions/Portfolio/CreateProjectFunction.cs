...
public class CreateProjectFunction(CreateProjectService service)
{
    ...
    {
        var model = await req.ReadFromJsonAsync<ProjectModel>(cancellationToken);
        if (model is null)
        ...
        return new CreatedResult($"/api/projects/{result.Value}", new { id = result.Value });
    }
}
